...
import { TIngredient, TIngredientQty } from '../utils/type';
import { CurrencyIcon, FormattedDate } from '@ya.praktikum/react-developer-burger-ui-components';
import { loadIngredients, getOrder } from '../../services/selectors';
    ...
    <main className={styles.main_container}>
      {order &&
        <>
          ...
          <p className={`text text_type_main-medium mb-3 ${styles.order_name}`}>
            {order!.name}
          </p>
          <p className={`text text_type_main-default mb-10 ${styles.status_order}`}>
            {orderStatus}
          </p>
          <p className={`text text_type_main-medium mb-2 ${styles.order_name}`}>
            {'Состав:'}
          </p>
          <section className={styles.fill_order}>
            {orderIngredients && orderIngredients.map((item, i: number) => {
              return (
                <li key={i} className={`mt-4 mr-6 ${styles.li}`}>
